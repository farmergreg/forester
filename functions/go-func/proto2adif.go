...
func writeAwards(awards []string, rec adif.Record, field adifield.ADIField) {
	writeString(rec, field, strings.Join(awards, ","))
}

func writeCredit(credit []*adifpb.Credit, rec adif.Record, field adifield.ADIField) {
...
func writeQsl(qsl *adifpb.Qsl, rec adif.Record, sent, sentDate, rcvd, rcvdDate adifield.ADIField) {
	if qsl == nil {
		return
	}
...
func writeString(rec adif.Record, adifField adifield.ADIField, value string) {
	if value == "" {
		return
	}
	rec[adifField] = value
...
func writeDate(rec adif.Record, adifField adifield.ADIField, value time.Time) {
	if value.Unix() == 0 {
		return
	}
	// YYYYMMDD
...
func writeTime(rec adif.Record, adifField adifield.ADIField, value time.Time) {
	if value.Unix() == 0 {
		return
	}
	// HHMMSS
